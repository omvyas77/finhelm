...
Measured on the 75-question golden set, multi_hop was the worst-performing question type, and
...
Two design constraints, both learned from the failures this project already logged:
  ...
  * It fails open. Any error, timeout, or unparseable reply returns the original question
    ...
    question, which is what the pipeline did before decomposition, successfully 41% of the
    time.
"""
...
# which is the pre-decomposition behaviour and answers correctly 41% of the time; an
# unnecessary split costs latency on every question forever. On the 75-question golden set
# this fires on 18 of the 20 two-span questions and skips 21 of the 34 single-span ones.
...
def decompose(question: str, max_sub_questions: int = 4,
    ...
    `timeout_s` is a hard cap on the planner call, and it was missing: Config had
