...
exactly the ones a tired human reviewer misses:
  ...
  1. **Unfindable spans.** A gold span whose document is absent, or which no chunk of that
     ...
     measured against a target no system could reach. The 75-question set is clean on this
...
def load_chunks() -> dict[str, list[str]]:
    ...
    `fixed` alone is enough: the ceiling check on the 75-question set found all three strategies identical
